...
configList = WorkBook.getRangeByShtName("捞文本配置", "A2:L2", "down").value
            ...
            with WorkBook(tarPath, wbName, app) as wb:
                ...
                for tansFile in transList:
                    try:
                        ...
                        if hasattr(dataDeal, 'transColList'):
                            delattr(dataDeal, 'transColList')
                    except:
                    ...
                    for row in oriData[1:]:
                        if dataDeal.transId != row[idColOrder]:
