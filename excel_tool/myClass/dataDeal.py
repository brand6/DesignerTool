...
class DataDeal():
    ...
    def getLastAddRow(self):
        for row in range(len(self.rngData) - 1, 0, -1):
            if self.isEmpty(self.rngData[row][0]) == False:
                return row + 1
    ...
    def getInsertRow(self, checkValue, checkCol=0):
        ...
        if self.isNumber(checkValue):
            ...
            for row in range(len(self.rngData) - 1, 0, -1):
                ...
                if compareValue != None and (self.isNumber(compareValue) == False or float(compareValue) <= checkValue):
                    r = row + 1
                    break
        else:
            for row in range(len(self.rngData) - 1, 0, -1):
                if self.rngData[row][checkCol] != None:
                    r = row + 1
                    break
    ...
    def strCheck(self):
        for col in range(len(self.rngData[0])):
            if self.rngData[0][col] != None:
                for row in range(len(self.rngData)):
                    if self.rngData[row][col] != None:
                        if self.isNumber(self.rngData[row][col]) == True:
                            pass
                        # 处理时间格式
                        ...
                        # 处理带逗号和冒号的数字，文本格式的时间
                        elif re.match(r"[^']\d*[:|,|/]", self.rngData[row][col]) != None:
                            self.rngData[row][col] = "'" + self.rngData[row][col][0:]
                        # 处理'开头的文本
                        elif re.match(r"'[^']", self.rngData[row][col]) != None:
                            self.rngData[row][col] = "'" + self.rngData[row][col][0:]
    ...
    # 根据列名获得列的位置
    @classmethod
    def getDataColOrder(cls, dataList, colStr, checkRow=0):
    ...
    # 判断是否有效
    @classmethod
    def isNumberValid(cls, content, checkNum=0):
    ...
    # 数字转汉字
    @classmethod
    def getChineseNum(cls, num):
        returnStr = ''
        numStr = cls.toStr(num)
        numList = list(numStr)
        numList.reverse()

        for i in range(len(numList)):
            numS = int(numList[i])
            returnStr = cls.numChar[numS] + cls.kinChar[i] + returnStr

        return returnStr

    # 重载分隔操作
    @classmethod
    def split(cls, content, sep):
        if content == None:
            return []
        else:
